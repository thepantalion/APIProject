...
listNode* newListNode(int beginKey, int endkey, char command, int opCode){
    ...
    if(command == 'd') temp -> gotSnapshot = true;
...
treeNode* cloneTree(treeNode* treeToClone){ //deprecated: consuma troppa memoria
...
int main(){
    ...
    //debug
    freopen("test20.txt", "r", stdin);
    freopen("output20.txt", "w", stdout);
    treeNode* debugRBroot;
    ...
    while(quitted == false){
            ...
            if(listNodeToOperate -> gotSnapshot == true){
                if(RBroot != RBnil) {
                    while(RBNodeToOperate != RBnil){
                        ...
                        size = strlen(RBNodeToOperate -> data);
                        RBDestNodeToOperate -> data = malloc(size * sizeof(char));
                        for(i = 0; i < size; i++) RBDestNodeToOperate -> data[i] = RBNodeToOperate -> data[i];
        ...
        */
        if(command == 'c' && (ind1 == maxKey + 1)) {            //questo particolare codice viene eseguito se la change avviene senza sovrascrittura di campi 'data' già esistenti
            ...
            while(currentKey <= ind2){
                ...
                if(currentKey <= maxKey){
                    ...
                    //free(RBNodeToOperate -> data);                        //potrebbe essere necessario per evitare problemi
        ...
        printf("---\n");
        inorderListWalk();
        if(command != 'p') RBInorderTreeWalk(RBroot);
        if(maxKey < 20) print_t(RBroot);
        fflush(stdout);
    }

...
treeNode* undo(treeNode* root) {
    ...
    while(urHead -> current != urHead -> operator){
        ...
        if(listNodeToOperate -> isValid == true){
            if(listNodeToOperate -> command == 'c' && listNodeToOperate -> isOverwrite == true){
                ...
                if(listNodeToOperate -> beginKey != 1){
                    RBMaxNodeOperated = RBSearch(root, listNodeToOperate -> beginKey - 1); //this is bullshit, sicuramente cambia durante una delete
        ...
        if(urHead -> operator != urHead -> first) urHead -> operator = urHead -> operator -> previous;
    }
    return root;
}
treeNode* redo(treeNode* root){
    ...
    while(urHead -> current != urHead -> operator){             //controllare queste condizioni nel caso di
...
int main(){
    ...
    //debug
    freopen("inputdebug.txt", "r", stdin);
    freopen("outputdebug.txt", "w", stdout);
    int numOpDebug = 0;
        ...
        if(command != 'u' && command != 'r' && command != 'q'){
            ...
            if(urHead -> operator != urHead -> current){
                ...
                if(size <= snaptime/2){                                                                    //siccome potrebbe risultare dispendioso ripristinare una snapshot
                    ...
                    if(urHead -> current -> opCode % snaptime > snaptime/2 && snapNodeToOperate != urHead -> lastSnap) snapNodeToOperate = snapNodeToOperate -> following;
                ...
                //se 'c','d' => urHead -> last = urHead -> current;
            }   // Undo/Redo
